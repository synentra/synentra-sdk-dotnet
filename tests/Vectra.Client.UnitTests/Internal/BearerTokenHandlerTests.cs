...
{
    private static IOptionsMonitor<VectraClientOptions> CreateMonitor(VectraClientOptions options)
    {
        var monitor = new StaticOptionsMonitor(options);
        return monitor;
    }

    private static HttpClient BuildPipeline(MockHttpMessageHandler innerHandler, VectraClientOptions options)
    {
    ...
    public async Task SendAsync_AttachesAuthorizationHeader_WhenTokenIsSet()
    {
        ...
        var client = BuildPipeline(inner, new VectraClientOptions { BaseUrl = "http://localhost", BearerToken = "test-token" });
    ...
    {
        var inner = new MockHttpMessageHandler(HttpStatusCode.OK);
        var client = BuildPipeline(inner, new VectraClientOptions { BaseUrl = "http://localhost", BearerToken = "sdk-token" });
    ...
    {
        var inner = new MockHttpMessageHandler(HttpStatusCode.OK);
        var client = BuildPipeline(inner, new VectraClientOptions { BaseUrl = "http://localhost", BearerToken = null });
    ...
    {
        var inner = new MockHttpMessageHandler(HttpStatusCode.OK);
        var client = BuildPipeline(inner, new VectraClientOptions { BaseUrl = "http://localhost", BearerToken = "   " });
    ...
    private sealed class StaticOptionsMonitor : IOptionsMonitor<VectraClientOptions>
    {
        public StaticOptionsMonitor(VectraClientOptions value) => CurrentValue = value;
        public VectraClientOptions CurrentValue { get; }
        public VectraClientOptions Get(string? name) => CurrentValue;
        public IDisposable? OnChange(Action<VectraClientOptions, string?> listener) => null;
    }
}
